...
sealed interface LightBarButton {
    ...
    /**
     * An arbitrary composable in an icon-sized slot — for a button whose "icon" needs something
     * [LightIcons]' flat vector drawables can't do, e.g. live text (a real font, rather than a
     * glyph traced into a fixed vector path). [content] is drawn centered in a
     * [sizeUnits]x[sizeUnits] box, same footprint as [LightIcon]/[Icon] above.
     */
    data class Custom(
        override val onClick: (() -> Unit)?,
        override val contentDescription: String? = null,
...
internal fun LightBarButtonView(
    ...
    when (button) {
        ...
        is LightBarButton.Custom -> {
            Box(
                modifier = baseModifier.size(button.sizeUnits.gridUnitsAsDp()),
                contentAlignment = Alignment.Center,
            ) {
                button.content()
            }
